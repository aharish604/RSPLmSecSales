...
public class ReturnOrderDetailsActivity extends AppCompatActivity implements View.OnClickListener, AdapterInterface<SalesOrderBean> {
    // data members
    ...
    private String mStrBundleDmsDivision = "";
    private String mStrBundleDmsDivisionDesc = "";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (bundleExtras != null) {
            ...
            if (soListBean == null) {
                ...
                mStrBundleDmsDivision = bundleExtras.getString(Constants.DmsDivision);
                mStrBundleDmsDivisionDesc = bundleExtras.getString(Constants.DmsDivisionDesc);
        ...
        //declare UI
        ConstantsUtils.initActionBarView(this, toolbar, true, getString(R.string.title_return_order_details), 0);
        ...
        if (!Constants.restartApp(ReturnOrderDetailsActivity.this)) {
            setUI();
            if (soListBean == null) {
    ...
    private Drawable getImageView() {
        ...
        try {
            delvStatusImg = SOUtils.displayStatusImage(salesOrderBean.getStatusID(), salesOrderBean.getDelvStatus(), ReturnOrderDetailsActivity.this);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    /*get ssro data from data valt*/
    private void getSODataFromDataValt() {
        try {
            SalesOrderBeanList.clear();
            SalesOrderBeanList.addAll(OfflineManager.getSSSODetailsListFromDataValt(mDeviceNo,this));
    ...
    private void setUI() {
        ...
        if (soListBean == null) {
            ...
            if (delvStatusImg != null) {
                ...
                Drawable drawable = SOUtils.getSODefaultDrawable(ReturnOrderDetailsActivity.this);
                if (drawable != null)
    ...
    private void setSOListData() {
        ...
        tvDate.setText(ConstantsUtils.convertDateIntoDisplayFormat(ReturnOrderDetailsActivity.this, soListBean.getOrderDate()));
        ...
        if (mDeviceNo.equalsIgnoreCase("")) {
            ...
            tvDmsDivisionDesc.setText(getString(R.string.po_details_display_value, mStrBundleDmsDivisionDesc, mStrBundleDmsDivision));
        }
        ...
        if (!TextUtils.isEmpty(soListBean.getPODate())) {
            tvCustomerPODateDesc.setText(ConstantsUtils.convertDateIntoDisplayFormat(ReturnOrderDetailsActivity.this, soListBean.getPODate()));
        ...
        if (!TextUtils.isEmpty(soListBean.getOrderDate())) {
            tvOrderDateDesc.setText(ConstantsUtils.convertDateIntoDisplayFormat(ReturnOrderDetailsActivity.this, soListBean.getOrderDate()));
       ...
       /* if (!TextUtils.isEmpty(soListBean.getPaymentTermDesc())) {
        ...
        delvStatusImg = SOUtils.displayStatusImage(soListBean.getStatus(), soListBean.getDelvStatus(), ReturnOrderDetailsActivity.this);
        ivDeliveryStatus.setImageDrawable(delvStatusImg);
        displayConditionItemDetails();
        SimpleRecyclerViewAdapter<SOItemBean> recyclerViewAdapter = new SimpleRecyclerViewAdapter<>(this, R.layout.ro_item_material, new AdapterInterface<SOItemBean>() {
            @Override
            ...
            public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup viewGroup, int i, View view) {
                return new RODetailsViewHolder(view, ReturnOrderDetailsActivity.this);
            }

            @Override
            public void onBindViewHolder(final RecyclerView.ViewHolder viewHolder, int i, final SOItemBean soItemBean) {
                ((RODetailsViewHolder) viewHolder).tvMaterialDesc.setText(soItemBean.getMatDesc());
                ((RODetailsViewHolder) viewHolder).tvRejectionReason.setText(soItemBean.getRejectionReasonDesc());
                ((RODetailsViewHolder) viewHolder).tvInvNo.setText(getString(R.string.lbl_inv_no)+" "+UtilConstants.removeLeadingZeros(soItemBean.getRefDocNo()!=null?soItemBean.getRefDocNo():""));
                ((RODetailsViewHolder) viewHolder).tvQty.setText(soItemBean.getSoQty() + " " + soItemBean.getUom());
                ((RODetailsViewHolder) viewHolder).tvAmount.setText(UtilConstants.getCurrencyFormat(soItemBean.getCurrency(), soItemBean.getNetAmount()));
                if (!TextUtils.isEmpty(soItemBean.getStatus())) {
                    Drawable statusImg = SOUtils.displayStatusImage(soItemBean.getStatus(), "", ReturnOrderDetailsActivity.this);
                    ((RODetailsViewHolder) viewHolder).ivDelvStatus.setImageDrawable(statusImg);
                } else {
                    ((RODetailsViewHolder) viewHolder).ivDelvStatus.setImageDrawable(delvStatusImg);
                }
                ((RODetailsViewHolder) viewHolder).clItem.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        if (viewCount == 1) {
                            if (((RODetailsViewHolder) viewHolder).clOrder.getVisibility() == View.GONE) {
                                ((RODetailsViewHolder) viewHolder).clOrder.setVisibility(View.VISIBLE);
                                ((RODetailsViewHolder) viewHolder).tvDelverQtyValue.setText(soItemBean.getDelvQty() + " " + soItemBean.getUom());
                                ((RODetailsViewHolder) viewHolder).tvPendingQtyValue.setText(soItemBean.getOpenQty() + " " + soItemBean.getUom());
                            } else {
                                ((RODetailsViewHolder) viewHolder).clOrder.setVisibility(View.GONE);
                            }
                        }
                    }
                });

                ((RODetailsViewHolder) viewHolder).clOrder.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        if (((RODetailsViewHolder) viewHolder).clOrder.getVisibility() == View.GONE) {
                            ((RODetailsViewHolder) viewHolder).clOrder.setVisibility(View.VISIBLE);
    ...
    private void setData() {
        try {
            ...
            tvDate.setText(mStrBundleDate);
            tvOrderType.setText(getString(R.string.po_details_display_value, mStrBundleOrderTypeDesc, mStrBundleOrderType));
            tvAmount.setText(UtilConstants.getCurrencyFormat(mStrBundleCurrency, mStrBundleAmount));
...
//        if (!mStrBundleStatus.equalsIgnoreCase("A") && !mStrBundleStatus.equalsIgnoreCase(""))
    ...
    private void onCancelSO() {
        showCancelDialog(ReturnOrderDetailsActivity.this);
    }

    /*displays alert with Message*/
    public void displayPopUpMsg() {
        AlertDialog.Builder builder = new AlertDialog.Builder(ReturnOrderDetailsActivity.this, R.style.MyTheme);
...
//        if (headerDetail != null) {
        ...
        MyUtils.dialogConformButton(ReturnOrderDetailsActivity.this, "Sales order # " + doc_no + " cancelled successfully", new DialogCallBack() {
            @Override
...
//                navtoPrevScreen();
  ...
  /*      Intent intentSOChange = new Intent(CollectionListDetailsActivity.this, SOChangeActivity.class);
    ...
    @Override
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup viewGroup, int i, View view) {
        return new RODetailsViewHolder(view, this);
    }

    @Override
    public void onBindViewHolder(RecyclerView.ViewHolder viewHolder, int i, SalesOrderBean salesOrderBean) {
        ((RODetailsViewHolder) viewHolder).tvMaterialDesc.setText(salesOrderBean.getMaterialDesc());
        try {
            ((RODetailsViewHolder) viewHolder).tvQty.setText(ConstantsUtils.checkNoUOMZero(salesOrderBean.getUom(), salesOrderBean.getQAQty()) + " " + salesOrderBean.getUom());
        } catch (OfflineODataStoreException e) {
            e.printStackTrace();
        }
        ((RODetailsViewHolder) viewHolder).tvAmount.setText(UtilConstants.getCurrencyFormat(salesOrderBean.getCurrency(), salesOrderBean.getNetAmount()));
        if (!TextUtils.isEmpty(salesOrderBean.getStatusID())) {
            delvStatusImg = SOUtils.displayStatusImage(salesOrderBean.getStatusID(), salesOrderBean.getDelvStatus(), ReturnOrderDetailsActivity.this);
            ((RODetailsViewHolder) viewHolder).ivDelvStatus.setImageDrawable(delvStatusImg);
        } else {
            Drawable drawable = SOUtils.getSODefaultDrawable(ReturnOrderDetailsActivity.this);
            ((RODetailsViewHolder) viewHolder).ivDelvStatus.setImageDrawable(drawable);
        }
    }
}

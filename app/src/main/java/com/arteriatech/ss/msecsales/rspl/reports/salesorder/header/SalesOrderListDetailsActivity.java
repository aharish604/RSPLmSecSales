...
//        if (tabPosition == Constants.TAB_POS_1 && !isSOChanged) {
        if (!Constants.restartApp(SalesOrderListDetailsActivity.this)) {
            setUI();
            if (soListBean == null) {
    ...
    private void getSODataFromDataValt() {
        try {
            try {
                ...
                if (OfflineManager.getSSSODetailsListFromDataValt(mDeviceNo,this)!=null&&!OfflineManager.getSSSODetailsListFromDataValt(mDeviceNo,this).isEmpty()) {
                    SalesOrderBeanList.addAll(OfflineManager.getSSSODetailsListFromDataValt(mDeviceNo,this));
                }
                ArrayList<SalesOrderConditionsBean> soConditionBean = null;
                if (SalesOrderBeanList!=null&&!SalesOrderBeanList.isEmpty()) {
                    soConditionBean = SalesOrderBeanList.get(0).getSalesOrderConditionsBeanArrayList();
                }
                salesOrderConditionsBeanArrayList.clear();
                if (soConditionBean!=null) {
    ...
    private void setSOListData() {
        ...
        tvDate.setText(ConstantsUtils.convertDateIntoDisplayFormat(SalesOrderListDetailsActivity.this, soListBean.getOrderDate()));
        ...
        if (!TextUtils.isEmpty(soListBean.getPODate())) {
            tvCustomerPODateDesc.setText(ConstantsUtils.convertDateIntoDisplayFormat(SalesOrderListDetailsActivity.this, soListBean.getPODate()));
        ...
        if (!TextUtils.isEmpty(soListBean.getOrderDate())) {
            tvOrderDateDesc.setText(ConstantsUtils.convertDateIntoDisplayFormat(SalesOrderListDetailsActivity.this, soListBean.getOrderDate()));
...
//                ((SODetailsViewHolder) viewHolder).tvQty.setText(soItemBean.getSoQty() + " " + soItemBean.getUom());
                ...
                ((SODetailsViewHolder) viewHolder).tvAmount.setText(UtilConstants.getCurrencyFormat(soItemBean.getCurrency(), soItemBean.getNetAmount()));
                if (!TextUtils.isEmpty(soItemBean.getStatus())) {
                    Drawable statusImg = SOUtils.displayStatusImage(soItemBean.getStatus(), "", SalesOrderListDetailsActivity.this);
                    ((SODetailsViewHolder) viewHolder).ivDelvStatus.setImageDrawable(statusImg);
                }else {
                ...
                if(!soItemBean.getHigherLevelItemno().equalsIgnoreCase("000000")){
                    ((SODetailsViewHolder) viewHolder).tvAmount.setText(UtilConstants.getCurrencyFormat(soItemBean.getCurrency(), "0.00"));
                    Drawable img = ContextCompat.getDrawable(SalesOrderListDetailsActivity.this, R.drawable.ic_shopping_cart_black_24dp).mutate();
                    if (img != null)
                        img.setColorFilter(ContextCompat.getColor(SalesOrderListDetailsActivity.this, R.color.colorGray), PorterDuff.Mode.SRC_IN);
                    try {
                        delvStatusImg = SOUtils.displayStatusImage(soItemBean.getStatus(), "", SalesOrderListDetailsActivity.this);
                        if (delvStatusImg != null)
                            delvStatusImg.setColorFilter(ContextCompat.getColor(SalesOrderListDetailsActivity.this, R.color.colorGray), PorterDuff.Mode.SRC_IN);
                        ((SODetailsViewHolder) viewHolder).ivDelvStatus.setImageDrawable(delvStatusImg);
                    } catch (Throwable e) {
                        try {
                ...
                if(soItemBean.getIsfreeGoodsItem().equalsIgnoreCase("X")){
                    Drawable img = ContextCompat.getDrawable(SalesOrderListDetailsActivity.this, R.drawable.ic_shopping_cart_black_24dp).mutate();
                    if (img != null)
                        img.setColorFilter(ContextCompat.getColor(SalesOrderListDetailsActivity.this, R.color.colorGray), PorterDuff.Mode.SRC_IN);
                    try {
...
//                navtoPrevScreen();
  ...
  /*      Intent intentSOChange = new Intent(CollectionListDetailsActivity.this, SOChangeActivity.class);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder viewHolder, int i, SalesOrderBean salesOrderBean) {
        ...
        if(mDeviceNo.equalsIgnoreCase("")){
            ((SODetailsViewHolder) viewHolder).tvAmount.setText(UtilConstants.getCurrencyFormat(salesOrderBean.getCurrency(), salesOrderBean.getNetAmount()));
        }else{
            if(salesOrderBean.getIsfreeGoodsItem().equalsIgnoreCase(Constants.X)){
                ((SODetailsViewHolder) viewHolder).tvAmount.setText(UtilConstants.getCurrencyFormat(salesOrderBean.getCurrency(), "0.00"));
            }else{
                ((SODetailsViewHolder) viewHolder).tvAmount.setText(UtilConstants.getCurrencyFormat(salesOrderBean.getCurrency(), salesOrderBean.getNetAmount()));
            }
        }
